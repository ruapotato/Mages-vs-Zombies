...
var server_info: Dictionary = {
	"name": "Mages vs Zombies Server",
	"map": "main",
	"max_players": MAX_CLIENTS
}

## Debug verbosity level (0=off, 1=basic, 2=detailed)
var debug_level: int = 2


func _ready() -> void:
	_debug_print(1, "[NetworkManager] Initializing...")
	multiplayer.peer_connected.connect(_on_peer_connected)
	multiplayer.peer_disconnected.connect(_on_peer_disconnected)
	multiplayer.connected_to_server.connect(_on_connected_to_server)
	multiplayer.connection_failed.connect(_on_connection_failed)
	multiplayer.server_disconnected.connect(_on_server_disconnected)
	_debug_print(1, "[NetworkManager] Ready - signals connected")


## Debug print helper
func _debug_print(level: int, message: String) -> void:
	if debug_level >= level:
		var timestamp := Time.get_ticks_msec()
		print("[%d] %s" % [timestamp, message])


func host_game(port: int = DEFAULT_PORT, player_name: String = "Host") -> Error:
	_debug_print(1, "[NetworkManager] Attempting to host game on port %d" % port)
	peer = ENetMultiplayerPeer.new()

	var error: Error = peer.create_server(port, MAX_CLIENTS)
	if error != OK:
		_debug_print(1, "[NetworkManager] ERROR: Failed to create server: %s" % error_string(error))
	...
	_debug_print(1, "[NetworkManager] Server started on port %d (peer_id=1, name='%s')" % [port, player_name])
	_debug_print(2, "[NetworkManager] Server state: max_clients=%d, connected_players=%s" % [MAX_CLIENTS, str(connected_players)])
...
func join_game(address: String, port: int = DEFAULT_PORT, player_name: String = "Player") -> Error:
	_debug_print(1, "[NetworkManager] Attempting to join game at %s:%d as '%s'" % [address, port, player_name])
	peer = ENetMultiplayerPeer.new()

	var error: Error = peer.create_client(address, port)
	if error != OK:
		_debug_print(1, "[NetworkManager] ERROR: Failed to create client: %s" % error_string(error))
	...
	_debug_print(1, "[NetworkManager] Client created, connecting to %s:%d..." % [address, port])
...
func _on_peer_connected(peer_id: int) -> void:
	_debug_print(1, "[NetworkManager] Peer connected: %d" % peer_id)

	if multiplayer.is_server():
		_debug_print(2, "[NetworkManager] Server sending lobby state to peer %d" % peer_id)
		# Send current lobby state to new player
		rpc_id(peer_id, "_receive_lobby_state", connected_players, server_info)


func _on_peer_disconnected(peer_id: int) -> void:
	_debug_print(1, "[NetworkManager] Peer disconnected: %d" % peer_id)

	if peer_id in connected_players:
		var player_info: Dictionary = connected_players[peer_id]
		_debug_print(2, "[NetworkManager] Removing player '%s' (peer %d) from lobby" % [player_info.get("name", "unknown"), peer_id])
...
func _on_connected_to_server() -> void:
	...
	_debug_print(1, "[NetworkManager] Connected to server! My peer_id=%d" % my_id)
	connection_succeeded.emit()

	# Send our player info to the server
	_debug_print(2, "[NetworkManager] Sending player info to server: %s" % str(local_player_info))
	rpc_id(1, "_register_player", local_player_info)


func _on_connection_failed() -> void:
	_debug_print(1, "[NetworkManager] ERROR: Connection failed!")
	peer = null
	multiplayer.multiplayer_peer = null
...
func _on_server_disconnected() -> void:
	_debug_print(1, "[NetworkManager] Server disconnected!")
	peer = null
	multiplayer.multiplayer_peer = null
	connected_players.clear()
	players_loaded.clear()
...
func _register_player(player_info: Dictionary) -> void:
	...
	_debug_print(1, "[NetworkManager] _register_player RPC from peer %d" % sender_id)

	if not multiplayer.is_server():
		_debug_print(2, "[NetworkManager] Ignoring _register_player - not server")
		return

	connected_players[sender_id] = player_info.duplicate()
	_debug_print(1, "[NetworkManager] Registered player '%s' (peer %d)" % [player_info.get("name", "unknown"), sender_id])
	_debug_print(2, "[NetworkManager] Total connected players: %d - %s" % [connected_players.size(), str(connected_players.keys())])
...
func _receive_lobby_state(players: Dictionary, info: Dictionary) -> void:
	_debug_print(2, "[NetworkManager] Received lobby state: %d players" % players.size())
