...
signal night_began()  # CRITICAL: Zombies are MUCH more dangerous at night
signal day_began()  # Day has started, zombies return to normal
...
var was_night: bool = false  # Track night state for signal emission
...
func _ready() -> void:
	...
	# Notify WeatherManager of initial time
	if WeatherManager:
		WeatherManager.set_time_of_day(current_hour, was_night)

	# Scene nodes will be found in _process when available

func _find_scene_nodes() -> void:
	# Find DirectionalLight3D (sun) and WorldEnvironment in current scene
	...
	if not sun_light:
		var lights = _find_nodes_of_type(root, "DirectionalLight3D")
		if lights.size() > 0:
			sun_light = lights[0]
			print("[DayNightCycle] Found sun: %s" % sun_light.name)

	if not world_environment:
		var envs = _find_nodes_of_type(root, "WorldEnvironment")
		if envs.size() > 0:
			world_environment = envs[0]
			print("[DayNightCycle] Found environment: %s" % world_environment.name)

	# Get sky shader material
	if world_environment and world_environment.environment and not sky_material:
		var env = world_environment.environment
		print("[DayNightCycle] Checking environment - bg_mode: %d, has sky: %s" % [env.background_mode, str(env.sky != null)])
		if env.sky:
			print("[DayNightCycle] Sky exists - has sky_material: %s" % str(env.sky.sky_material != null))
			if env.sky.sky_material:
				print("[DayNightCycle] Sky material type: %s" % env.sky.sky_material.get_class())
...
func _process(delta: float) -> void:
	...
	# Check day/night transitions (CRITICAL for zombie behavior)
	var is_currently_night = is_night()
	if is_currently_night != was_night:
		was_night = is_currently_night
		if is_currently_night:
			night_began.emit()
			print("[DayNightCycle] NIGHT HAS FALLEN - Zombies are now MUCH more dangerous!")
		else:
			day_began.emit()
			print("[DayNightCycle] Day has begun - Zombies return to normal strength")

	# Notify WeatherManager of time changes
	if WeatherManager:
		WeatherManager.set_time_of_day(current_hour, is_currently_night)
...
## (Removed Equipment/ArmorData dependency - always returns false for now)
func _player_has_cyclops_eye() -> bool:
	# TODO: Implement equipment system check when ready
	return false
